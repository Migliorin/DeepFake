...
from tensorflow.keras.layers import Dense, Flatten, BatchNormalization, Dropout, Conv3D, MaxPooling3D, LSTM, TimeDistributed, MaxPooling2D
...
class ImageTensor():
    ...
    def switch_image_tensor_49_01(self)->None:
        self.switch_image_tensor_49()

        self.epochs = 32

    def switch_image_tensor_49_02(self)->None:
        self.frames = 20
        self.n_frames = 20
        self.path = "../datasets/dfdc_train_part_49"
        self.path_plots = "../plots"
...
class PreProcessVideo():
    ...
    def pre_process_video_with_color(self,path_video)->list:
        frames = []
        vidcapture = cv2.VideoCapture(path_video)
        index = 0
        j = 0
...
class BatchData():
    ...
    def getBatchDataWithColor(self,batch:int)->tuple:
        
        batch_size = self.preProcessVideo.image_tensor.batch_size
        # dimensions
        batch_data = [] # batch data that will pass forward
        
        batch_labels = [] # batch labels that will pass forward
        ...
        for name,label in zip(name_list,label_list):
            path_ = f"{self.preProcessVideo.image_tensor.path}/{name}"
            batch_data.append(self.preProcessVideo.pre_process_video_with_color(path_))
    ...
    def generatorWithColor(self):
        ...
        while True:
            ...
            for batch in range(num_batches): # we iterate over the number of batches
                yield self.getBatchDataWithColor(batch)
...
class Models():
    ...
    def create_checkpoint_02(self,name:str)->list:
        curr_dt_time = datetime.datetime.now()
    ...
    def InceptionV3_artigo(self) -> Sequential:
        ...
        modelIv3 = Sequential([
            TimeDistributed(model, input_shape=inputShape)
        ], name="inception_lstm")
        ...
        modelIv3.compile(optimizer=OPTIMIZER,metrics=['accuracy'],loss='categorical_crossentropy')
...
class SavePlots():
    ...
    def plotModelHistory_02(self,h:Sequential)->None:
        path = self.image_tensor.path_plots
        ...
        try: 
            ...
            with open(f'{path}/{self.name_save}.json', 'w+', encoding='utf-8') as f:
                json.dump({
                    "accuracy": float(max(h.history['accuracy'])),
                    "val_accuracy": float(max(h.history['val_accuracy']))
                },
                f,
                ensure_ascii=False,
                indent=4)
